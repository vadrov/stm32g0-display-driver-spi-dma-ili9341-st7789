					   ...
					   )
{
	...
	if (hdma->dma) {
		...
		if (lcd->data_bus == LCD_DATA_8BIT_BUS) {
			dma_x->CCR &= ~(DMA_CCR_MSIZE | DMA_CCR_PSIZE);
			dma_x->CCR |= LL_DMA_MDATAALIGN_BYTE | LL_DMA_PDATAALIGN_BYTE;
		}
		else if (lcd->data_bus == LCD_DATA_16BIT_BUS) {
			dma_x->CCR &= ~(DMA_CCR_MSIZE | DMA_CCR_PSIZE);
			dma_x->CCR |= LL_DMA_MDATAALIGN_HALFWORD | LL_DMA_PDATAALIGN_HALFWORD;
		}
